...
public:
    ...
    void remove(const int nData);
    bool find(const int nData);
    void Display() 
    {
        PrintTree(root);
    }
...
void RBTree::insert(const int nData)
{
...
void RBTree::remove(const int nData)
{
...
bool RBTree::find(const int nData)
{
    return (FindNode(nData) == nil) ? false : true;
}
